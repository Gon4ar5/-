...
namespace IT
{
	...
	void Add(IdTable &idtable, Entry entry)
	{
		...
		idtable.table[idtable.size].value.vWRD.len = NULL; // длина строки 0
		idtable.table[idtable.size].value.vWRD.str[0] = TI_STR_DEFAULT; // пустая строка
		idtable.size++;
	}
	...
	void SetValue(IdTable &idtable, int indTI, char* token) // установка значения переменной
	{
		...
		{
			idtable.table[indTI].value.vWRD.len = strlen(token);
			strcpy_s(idtable.table[indTI].value.vWRD.str, TI_STR_MAXSIZE, token);
		}
	}
}